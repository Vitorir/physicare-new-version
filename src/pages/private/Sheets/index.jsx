...
const PerimetriaForm = () => {
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      const fichaPerimetriaRef = await addDoc(collection(firestore, 'fichasPerimetria'), {
        patientUid,
        objetivo,
        altura,
        peso,
      ...
      console.log("Ficha de perimetria adicionada com sucesso!: ", fichaPerimetriaRef.id);
    } catch (error) {
  ...
  const handleInputChange = (measureName, value) => {
    setMeasurements((prevMeasurements) => {
      ...
      if (index !== -1) {
        // Se a medida já existir, atualize o valor
        updatedMeasurements[index].value = value;
      } else {
        // Se não existir, adicione uma nova medida
